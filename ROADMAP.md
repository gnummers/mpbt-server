...
| RE weapon fire packets | 🔧 | v1.23 client → server fire request is now partially decoded in RESEARCH.md §19.3: `Combat_SendCmd12Action_v123` emits `cmd 12`, and action `0` is the gated weapon-fire request from `Combat_InputActionDispatch_v123`. Server → client hit/damage result still unknown; `Cmd68` is the current projectile/effect candidate. |
